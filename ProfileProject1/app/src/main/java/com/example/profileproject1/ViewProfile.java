...
public class ViewProfile extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        profileInfo = v.findViewById(R.id.lblProfileInfo);

        Bundle bundle = this.getArguments();
        String name_str =  bundle.getString("name");
        name.setText(name_str);

        String profileInfo_str =  bundle.getString("aboutInfo");
        profileInfo.setText(profileInfo_str);

        btnProjects.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                ViewProjects f3 = new ViewProjects();

                getFragmentManager().beginTransaction().replace(R.id.frameLay_Main, f3).commit();
            }
        });
